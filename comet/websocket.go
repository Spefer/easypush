...
import (
	...
	itime "github.com/Spefer/common/libs/time"
	"errors"
	"fmt"
)
...
func (server *Server) subscribeWS(conn *websocket.Conn) {
	...
	if p, err = ch.ProtoRing.Set(); err == nil {
		if userid, topics, token, err = server.wsSubscribe(conn, p); err == nil {
			...
			log.Errorf("wsSubscribe failed user(%s) failed : %v", userid, err)
		}
	}
	if err != nil {
		conn.Close()
	...
	if err = server.UnSubscribe(disArg); err != nil {
		log.Errorf("UnSubscribe user(%s)  failed : %v", userid, err)
	}
	if isDebug {
		log.Debugf("userid: %s server websocket goroutine exit", userid)
	}
	return
}

func (server *Server) processWriteWebsocket(userid string, conn *websocket.Conn, ch *Channel) {
	var (
		p   *proto.Proto
		err error
	)
...
func (server *Server) wsSubscribe(conn *websocket.Conn, p *proto.Proto) (userid string, topics []string, token string, err error) {
	if err = p.ReadWebsocket(conn); err != nil {
		log.Errorf("error happen: %s", err)
		return
	}
	if p.OpCode != define.OP_SUB_REQ {
		err = errors.New(fmt.Sprintf(ErrOperationStr, define.OP_SUB_REQ, p.OpCode))
		return
	}

	sub := new(proto.SubREQ)
	log.Debugf("body:%s", string(p.Body))
