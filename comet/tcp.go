...
"github.com/Spefer/common/libs/bufio"
"github.com/Spefer/common/libs/bytes"
...
func (server *Server) processWriteTcp(userid string, conn *net.TCPConn, wr *bufio.Writer, wp *bytes.Pool, wb *bytes.Buffer, ch *Channel) {
	var (
		p   *proto.Proto
		err error
	)
...
// auth
func (server *Server) tcpSubscribe(rr *bufio.Reader, wr *bufio.Writer, p *proto.Proto) (userid string, topics []string, token string, err error) {
	if err = p.ReadTCP(rr); err != nil {
		return
	}
