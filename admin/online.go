...
func (n *NotifyChannel) subDo(uid string) {
...
func (n *NotifyChannel) Notify(userid string) {
	n.nchan <- userid
}

func NewNotifyChannel(size int32) *NotifyChannel {
	n := new(NotifyChannel)
	n.nchan = make(chan string, size)
	go n.Do()
	return n
}
...
func (n *OnlineNotify) Notify(userid string) {
	idx := hash.ToHashNum(userid) & chanmod
