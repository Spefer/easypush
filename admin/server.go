...
import (
	...
	"github.com/Spefer/common/libs/hash"
)
type Server struct {
...
func (s *Server) Sub(userid string, serverid int32, topics []string) error {
	return s.GetBucket(userid).Put(userid, serverid, topics)
}

func (s *Server) UnSub(userid string, serverid int32) error {
	return s.GetBucket(userid).Del(userid, serverid)
}

func (s *Server) GetBucket(userid string) *Bucket {
	idx := hash.ToHashNum(userid) % s.BucketSize
	return s.Buckets[idx]
}

func (s *Server) GetTopicServers(topic string) []int32 {
	var servers []int32
