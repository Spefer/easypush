...
type ServerSession struct {
	userSession map[string]*Session //userId
	topicCount  map[string]int64    //topicId
}

func NewServerSession() *ServerSession {
	s := new(ServerSession)
	s.userSession = make(map[string]*Session)
	s.topicCount = make(map[string]int64)
	return s
}

// NewSession new a session for a user
func NewSession(server int32, topics []string) *Session {
	s := new(Session)
	s.ServerId = server
	s.Topics = topics
	return s
...
type Bucket struct {
	sync.RWMutex
	topicServers  map[string]map[int32]struct{} // map[topic]map[serverid]struct{}
	serverSession map[int32]*ServerSession
	userServer    map[string]int32 // userid -> serverid
	topicCount    map[string]int64 //  map[topic]userCount topic should be a string type
...
func NewBucket(conf ConfigBucket) *Bucket {
	b := new(Bucket)
	b.topicServers = make(map[string]map[int32]struct{})
	b.topicCount = make(map[string]int64)
	b.serverSession = make(map[int32]*ServerSession)
	b.userServer = make(map[string]int32)
...
func (b *Bucket) counter(userId string, server int32, topics []string, incr bool) {
	if incr {
...
func (b *Bucket) GetTopicServers(topic string) []int32 {
	var servers []int32
	b.RLock()
...
func (b *Bucket) Put(userId string, server int32, topics []string) error {
...
func (b *Bucket) Del(userId string, server int32) error {
	var (
		si *ServerSession
		s  *Session
		ok bool
...
func (b *Bucket) GetUserServer(userid string) (server int32, err error) {
	server = -1
...
func (b *Bucket) delEmpty(userId string, serverId int32) {
	//if isDebug {
...
func (b *Bucket) GetUsers(serverid int32) []*User {
	...
	if se, ok := b.serverSession[serverid]; ok {
		for userId, s := range se.userSession {
			u := &User{userId, s.Topics}
