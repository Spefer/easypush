...
type Cleaner struct {
	...
	size     int
	dataList *list.List
	maps     map[string]*list.Element //userid->list.Element
}
...
func NewCleaner(cleaner int) *Cleaner {
	...
	c.dataList = list.New()
	c.size = 0
	c.maps = make(map[string]*list.Element, cleaner) //userid->list.Element
	return c
}

func (c *Cleaner) PushFront(user string, server int32, expire time.Duration) {
	c.Lock()
...
func (c *Cleaner) Remove(key string) {
	c.Lock()
