...
import (
	...
	"github.com/Spefer/common/libs/nocopy"
...
type Storage interface {
	...
	SaveMMsg(userid []string, msg *proto.PushMsg) ([]string, error) //return value: []string->failedUsers
	DelMsg(msg *proto.PushMsg) error
	GetMsg(userid string, start int64, stop int64) ([]*proto.PushMsg, error)
	Size(userid string) int64
	UnInit()
	UpdateConfig()
}
...
func (r *RedisStorage) SaveMsg(msg *proto.PushMsg) (error) {
	...
	r.RLock()
	defer r.RUnlock()
	//change uid from int64 to string already, so here use userId directly
	//uid := strconv.FormatInt(msg.UserId, 10)
	redisnode, err := r.consistentHash.Get(msg.UserId)
	if err != nil {
		return err
	}
	...
	client.ZAdd(msg.UserId, redis.Z{Score: float64(msg.Id), Member: data})

	return nil
}

func (r *RedisStorage) GetMsg(userid string, start int64, stop int64) ([]*proto.PushMsg, error) {

	r.RLock()
	defer r.RUnlock()
	//use userId directly
	//uid := strconv.FormatInt(userid, 10)
	redisnode, err := r.consistentHash.Get(userid)
	if err != nil {
		return nil, err
	}
	client, ok := r.redisCluster[redisnode]
	...
	existdo := client.Exists(userid)
	if (!existdo.Val()) {
		return nil, nil
	}

	rangedo := client.ZRange(userid, start, stop)
	if rangedo.Err() != nil {
...
func (r *RedisStorage) Size(userid string) int64 {
	r.RLock()
	defer r.RUnlock()
	//use userid directly
	//uid := strconv.FormatInt(userid, 10)
	...
	do := client.ZCount(userid, "0", r.maxKey)
	if do.Err() != nil {
		return 0
	}
	return do.Val()
...
func (p *RedisStorage) DelMsg(msg *proto.PushMsg) error {
	...
	redisnode, err := p.consistentHash.Get(msg.UserId)
	if err != nil {
	...
	cmd := client.ZRemRangeByScore(msg.UserId, min, min)
	if cmd.Err() != nil {
		log.Errorln(cmd.Err())
	}
	return cmd.Err()
...
// @TODO :	1. optimize
//			2. transactions
func (r *RedisStorage) SaveMMsg(userid []string, msg *proto.PushMsg) ([]string, error) {
	r.RLock()
	defer r.RUnlock()
