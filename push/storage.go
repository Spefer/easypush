...
type Storage interface {
	...
	DelMsg(user string, id int64) error
}
...
func (p *RedisStorage) DelMsg(userId string, id int64) error {

	p.RLock()
	defer p.RUnlock()
	
	
	redisnode, err := p.consistentHash.Get(userId)
	if err != nil {
		log.Errorf("RedisStorage.DelMsg(%d, %d) failed : %v", userId, id, err)
		return err
	}
	...
	cmd := client.ZRemRangeByScore(userId, min, min)
	if cmd.Err() != nil {
		log.Errorln(cmd.Err())
	}
	return cmd.Err()
